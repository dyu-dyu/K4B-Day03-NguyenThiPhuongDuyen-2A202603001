        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                if obs_data.get("status") == "SUCCESS":
                    if "available_slots" in obs_data:
                        final_answer = (
                            f"Bác sĩ {obs_data.get('doctor', '')} thuộc chuyên khoa "
                            f"{obs_data.get('specialty', '')} tại {obs_data.get('hospital', '')}. "
                            f"Ngày {obs_data.get('date', '')}, các khung giờ còn trống: "
                            f"{', '.join(obs_data.get('available_slots', []))}."
                        )

                    elif "working_schedule" in obs_data:
                        schedule = obs_data.get("working_schedule", {})
                        schedule_text = "; ".join(
                            f"{date}: {', '.join(slots)}"
